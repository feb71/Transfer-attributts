...
# UI
# -----------------------------
st.title("VA-linje matcher (stabil v5): teoretisk → innmålt (Excel → Excel)")
...
if run_btn:
    ...
    # Auto-detect critical columns (prevents typos like "Nr" vs "Nr.")
    ...
    # Representatives for attributes
    theo_rep = group_rep(theo_df, gid_col)
    meas_rep = group_rep(meas_df, gid_col)

    # Transfer columns: everything from theoretical except technical point columns
    ...
    # Restore Gemini-like ID output (use the measured ID column name)
