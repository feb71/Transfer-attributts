...
st.set_page_config(page_title="VA-linje matcher (stabil v6)", layout="wide")
...
with st.expander("Match-regler", expanded=True):
    ...
    rules_df = st.data_editor(
        pd.DataFrame([
            {"name":"VEAS_VA.Type og Dimensjon","mode":"trim_upper"},
...
if run_btn:
    ...
    # Representative rows for attribute comparison
    theo_rep = group_rep(theo_df, gid_col)
    meas_rep = group_rep(meas_df, gid_col)

    # Transfer columns: everything from theoretical except technical columns
    ...
    # Shapely 1/2 compatibility mapping
    ...
    for mgid, mgeom, _ in meas_lines:
        ...
        # 1) Buffer polygon around measured line
        buf = mgeom.buffer(max_dist)

        # 2) Candidate query: returns indices (Shapely 2) or geometries (Shapely 1)
        q = tree.query(buf)
        ...
        for idx in cand_idxs:
            ...
            # 3) Distance check (safety)
            d = float(mgeom.distance(tg))
            if d > max_dist:
                continue

            # 4) Attribute matching (count how many selected fields match)
            ...
            # 5) Pick best candidate by smallest distance, then more attribute matches
            score = (d, -mcount)
        ...
        # 6) Transfer attributes to ALL points on measured line
